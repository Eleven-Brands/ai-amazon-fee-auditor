...
**Project:** AI Amazon Fee Auditor
**Researched:** 2026-05-27
**Confidence note:** All external network tools are restricted in this environment. Findings are based on training data (cutoff August 2025) and the project's declared stack in PROJECT.md. Version numbers are accurate as of August 2025 — pin verification via `pip index versions <package>` is recommended before first deploy.
...
**Agent pattern:** Single-agent with tool use (as stated in PROJECT.md). The agent receives a summary of anomalies, calls tools for ClickUp posting, and returns structured JSON. No LangGraph, no CrewAI, no AutoGen — the project explicitly defers multi-agent decomposition.

Tool use schema: Anthropic `tools` parameter with JSON Schema definitions. Use `tool_choice: {"type": "auto"}` for the main loop; switch to `tool_choice: {"type": "tool", "name": "post_clickup_comment"}` when forcing the final output step.
...
**Power BI access pattern — important detail:**
The project references a `powerbi-query` skill. In n8n, this is likely an HTTP Request node hitting the Power BI REST API Execute Queries endpoint. In the Python agent, replicate this as a `query_powerbi(dax_query: str) -> dict` tool function. Do NOT use `powerbiclient` PyPI package — it is an unofficial library with sparse maintenance. Use raw `requests` + `msal`.

DAX query pattern for fee data:
```
EVALUATE
SUMMARIZECOLUMNS(
    'FeeData'[ASIN],
    'FeeData'[Period],
    "FBAFee", SUM('FeeData'[FBA Fee]),
    "StorageFee", SUM('FeeData'[Storage Fee])
)
ORDER BY 'FeeData'[ASIN], 'FeeData'[Period]
```
...
**What NOT to use for anomaly detection:**
...
**ClickUp comment format constraint:** PROJECT.md states comments must be concise. Target: structured plain text with emoji flags, max ~20 lines. Verbose data goes in the attachment only.
...
```
Schedule Trigger (weekly, Monday 08:00) 
  → HTTP Request node (POST to agent trigger endpoint)
  → IF node (check agent response status)
  → [success] Set node (log result)
  → [error] Send notification (ClickUp comment or email)
```
...
**Fallback (per PROJECT.md):** If n8n/Cloud Run is unavailable, Windows Task Scheduler + `python agent.py` directly. This should be possible without code changes — the agent script must be runnable locally with environment variables set.
...
anthropic>=0.28.0          # tool_use + structured outputs
